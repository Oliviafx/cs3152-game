...
//		System.out.println("resize interior");
	...
	public void initialize(float[] pos, float[] size, float[] pad, String debugClr, TextureRegion tex, float height, float offsetx, float offsety) {
		if (tex != null) {
			setTexture(tex);
			offsety -= tex.getRegionHeight()/6/64;
		}
	...
	public void draw(ObstacleCanvas canvas) {
		...
		if (region != null) {
			...
			canvas.draw(texture,Color.WHITE,origin.x,origin.y,getX()*drawScale.x,getY()*drawScale.y+texture.getRegionHeight()/5, getAngle(),GameController.TEMP_SCALE * 2, GameController.TEMP_SCALE  * 2);
