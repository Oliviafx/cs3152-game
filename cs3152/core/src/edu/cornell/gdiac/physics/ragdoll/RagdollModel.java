...
public class RagdollModel extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		//body and torso connector
		RevoluteJointDef revoluteDef = new RevoluteJointDef();
		revoluteDef.bodyA = bodies.get(PART_HEAD).getBody();
		revoluteDef.bodyB = bodies.get(PART_BODY).getBody();
		revoluteDef.localAnchorA.set(0, -TORSO_OFFSET/2);
		revoluteDef.localAnchorB.set(0, TORSO_OFFSET/2);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		revoluteDef.lowerAngle = (float)-Math.PI/2;
		Joint joint = world.createJoint(revoluteDef);
		joints.add(joint);


		//torso and arm connectors
		revoluteDef = new RevoluteJointDef(); //RIGHT ARM
		revoluteDef.bodyA = bodies.get(PART_BODY).getBody();
		revoluteDef.bodyB = bodies.get(PART_RIGHT_ARM).getBody();
		revoluteDef.localAnchorA.set(ARM_XOFFSET/2, ARM_YOFFSET/2 + ((BoxObstacle)bodies.get(PART_RIGHT_ARM)).getHeight()/2);
		revoluteDef.localAnchorB.set(ARM_XOFFSET/2, 0);
		joint = world.createJoint(revoluteDef);
		joints.add(joint);


		revoluteDef = new RevoluteJointDef(); //LEFT ARM
		revoluteDef.bodyA = bodies.get(PART_BODY).getBody();
		revoluteDef.bodyB = bodies.get(PART_LEFT_ARM).getBody();
		revoluteDef.localAnchorA.set(-ARM_XOFFSET/2, ARM_YOFFSET/2 + ((BoxObstacle)bodies.get(PART_LEFT_ARM)).getHeight()/2);
		revoluteDef.localAnchorB.set(ARM_XOFFSET/2,0);
		joint = world.createJoint(revoluteDef);
		joints.add(joint);


		//torso and thigh connectors
		revoluteDef = new RevoluteJointDef(); //right thigh
		revoluteDef.bodyA = bodies.get(PART_BODY).getBody();
		revoluteDef.bodyB = bodies.get(PART_RIGHT_THIGH).getBody();
		revoluteDef.localAnchorA.set(THIGH_XOFFSET/2 + ((BoxObstacle)bodies.get(PART_RIGHT_THIGH)).getWidth()/2, -THIGH_YOFFSET/2);
		revoluteDef.localAnchorB.set(THIGH_XOFFSET/2,THIGH_YOFFSET/2);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		revoluteDef.lowerAngle = (float)-Math.PI/2;
		joint = world.createJoint(revoluteDef);
		...
		revoluteDef = new RevoluteJointDef(); //left thigh
		revoluteDef.bodyA = bodies.get(PART_BODY).getBody();
		revoluteDef.bodyB = bodies.get(PART_LEFT_THIGH).getBody();
		revoluteDef.localAnchorA.set(-THIGH_XOFFSET/2 - ((BoxObstacle)bodies.get(PART_LEFT_THIGH)).getWidth()/2, -THIGH_YOFFSET/2);
		revoluteDef.localAnchorB.set(-THIGH_XOFFSET/2,THIGH_YOFFSET/2);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		revoluteDef.lowerAngle = (float)-Math.PI/2;
		...
		//arm connectors
		revoluteDef = new RevoluteJointDef(); //right arm
		revoluteDef.bodyA = bodies.get(PART_RIGHT_ARM).getBody();
		revoluteDef.bodyB = bodies.get(PART_RIGHT_FOREARM).getBody();
		revoluteDef.localAnchorA.set(-FOREARM_OFFSET/2, 0);
		revoluteDef.localAnchorB.set(FOREARM_OFFSET/2,0);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		revoluteDef.lowerAngle = (float)-Math.PI/2;
		joint = world.createJoint(revoluteDef);
		...
		revoluteDef = new RevoluteJointDef(); //left arm
		revoluteDef.bodyA = bodies.get(PART_LEFT_ARM).getBody();
		revoluteDef.bodyB = bodies.get(PART_LEFT_FOREARM).getBody();
		revoluteDef.localAnchorA.set(-FOREARM_OFFSET/2, 0);
		revoluteDef.localAnchorB.set(FOREARM_OFFSET/2, 0);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		...
		//shin and thigh connectors
		revoluteDef = new RevoluteJointDef(); //right thigh
		revoluteDef.bodyA = bodies.get(PART_RIGHT_THIGH).getBody();
		revoluteDef.bodyB = bodies.get(PART_RIGHT_SHIN).getBody();
		revoluteDef.localAnchorA.set(0, -SHIN_OFFSET/2);
		revoluteDef.localAnchorB.set(0,SHIN_OFFSET/2);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
		revoluteDef.lowerAngle = (float)-Math.PI/2;
		joint = world.createJoint(revoluteDef);
		...
		revoluteDef = new RevoluteJointDef(); //LEFT thigh
		revoluteDef.bodyA = bodies.get(PART_LEFT_THIGH).getBody();
		revoluteDef.bodyB = bodies.get(PART_LEFT_SHIN).getBody();
		revoluteDef.localAnchorA.set(0, -SHIN_OFFSET/2);
		revoluteDef.localAnchorB.set(0,SHIN_OFFSET/2);
		revoluteDef.enableLimit = true;
		revoluteDef.upperAngle = (float)Math.PI/2;
